...
{
    public class OscodaTownshipMI : City
    {
        private List<string> docUrls = null;

        public OscodaTownshipMI()
        {
            cityEntity = new CityInfo()
            {
                CityId = "OscodaTownshipMI",
                CityName = "Oscoda Township",
                CityUrl = "http://www.oscodatownshipmi.gov/1/322/index.asp",
                StateCode = "MI"
            };
            ...
            this.docUrls = File.ReadAllLines("OscodaTownshipMI_Urls.txt").ToList();
        }
                ...
                foreach (string url in this.docUrls)
                {
                    ...
                    if (fileTokens != null)
                    {
                        foreach (var fileToken in fileTokens)
                        {
                            ...
                            DateTime meetingDate = string.IsNullOrEmpty(meetingDateText) ? DateTime.Parse(string.Format("{0}-1-1", i)) : DateTime.Parse(meetingDateText);
                            if (meetingDate < this.dtStartFrom)
                            {
                                Console.WriteLine("Too early, skip...");
                                continue;
                            }
                            string fileUrl = "https://oscodatwpmi.documents-on-demand.com" + fileToken.SelectToken("$..href").ToString();
